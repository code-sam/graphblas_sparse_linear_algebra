...
pub trait Semiring<Multiplier, Multiplicant, Product, EvaluationDomain>
where
...
macro_rules! implement_semiring_operator {
    ($semiring:ident, $addition_operator:ident, $multiplication_operator:ident, $multiplier:ty, $multiplicant:ty, $product:ty, $evaluation_domain:ty,
        $graphblas_operator:ident) => {
        impl Semiring<$multiplier, $multiplicant, $product, $evaluation_domain>
            for $semiring<$multiplier, $multiplicant, $product, $evaluation_domain>
        {
            fn graphblas_type(&self) -> GrB_Semiring {
        ...
        impl $semiring<$multiplier, $multiplicant, $product, $evaluation_domain> {
            pub fn new() -> Self {
                Self {
                    addition_operator:
                        $addition_operator::<$product, $product, $product, $product>::new(),
...
#[derive(Debug, Clone)]
pub struct PlusTimes<Multiplier, Multiplicant, Product, EvaluationDomain>
where
    Multiplier: ValueType,
    Multiplicant: ValueType,
    Product: ValueType,
    EvaluationDomain: ValueType,
{
    ...
    addition_operator: Plus<Product, Product, Product, Product>,
    multiplication_operator: Times<Multiplier, Multiplicant, Product, EvaluationDomain>,
}
...
mod tests {
    ...
    fn test_new_binary_operator() {
        let _plus = Plus::<i8, i8, i8, i8>::new();
    }
}
