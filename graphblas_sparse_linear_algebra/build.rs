...
fn path_with_graphblas_implementation() -> PathBuf {
    ...
    path_with_graphblas_implementation.push("SuiteSparse_GraphBLAS");
...
fn build_and_link_dependencies() {
    ...
    match path_with_graphblas_header_file.try_exists() {
        Err(error) => {
            panic!("Unable to check if path {} exists, got error: {}", path_with_graphblas_header_file.display(), error)
        }
        Ok(does_exist) => {
            if !does_exist {
                ...
                graphblas_repo = match Repository::open(path_with_graphblas_implementation.clone()) {
                    Ok(repo) => repo,
                    Err(error) => panic!("failed to open SuiteSparse GraphBLAS repository: {}", error),
                };
            }
        }
    };

    // https://stackoverflow.com/questions/72694616/cloning-from-a-particular-commit-sha-using-git2-rust
    // https://stackoverflow.com/questions/55141013/how-to-get-the-behaviour-of-git-checkout-in-rust-git2
    let obj: Object = graphblas_repo.find_commit(Oid::from_str("97510b55fba589e6ea315fe433237633057e7048").unwrap()).unwrap().into_object();
...
fn build_static_graphblas_implementation(cargo_build_directory: &OsString) {
    let _dst = cmake::Config::new("graphblas_implementation/SuiteSparse_GraphBLAS")
        .define("JOBS", "32") // TODO: check number of available cores
